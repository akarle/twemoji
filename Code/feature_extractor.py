...
class FeatureExtractor():
    ...
    def __init__(self):
        ...
        self.features_dict = {'unary': self.unary_features,
                              'sent_analysis': self.sent_analysis}
    ...
    def sent_analysis(self, data, sent_pred_func):
        ...
        # Possible TODO: can move sent_clf to be an instance variable...
        # I think it's cleaner to have it isolated here, BUT if we end up
        # calling sent_analysis mult times, then it will retrain sent_clf
        # each time... (not an issue if this func used once)
        clf = MultinomialNB()
        sent_clf = \
            SentimentClassifier(sent_pred_func,
                                os.path.join('..', 'Data', 'sent140', 'raw'),
                                clf)
    ...
    def extract_features(self, data, features_to_extract):
        """ External function to call to extract multiple features
            ...
            ie:
                ...
                would be 'unary' feats on data AND 'sent_analysis' of
                type 'baseline' on data
        """
